			...
			methods: {
				...
				delBtn(event) {
					...
					if(this.firstSubmit) {
						...
						//alert(props)
						//return ;
                        this.$http.post('BasicMode/CompanyIntroDuctionDelete?' + props).then(response => {
							// success callback
							var data = response.body;
							//error
							if(data.state === "error") {
				...
				getData: function(cd) {
					this.$http.get('BasicMode/CompanyIntroDuctionShow?rows=100&page=1&sidx=CreatorTime desc&records=100').then(response => {
						//loading.removeLoading("#pattern-table");
						var data = response.body;
						//error
						if(data.state === "error") {
